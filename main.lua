...
--[[
	Called just once at the beinning of the game; used to set up the 
	game objects, varaibles, etc. and prepare the game world.
]]
function love.load()
	--set love's default filter to "nearest-neighbor", which essentially
	-- means there will be no filtering of pixels (blurriness), which is
	-- important for a nice crisp, 2D looks
	...
	-- the state machien we'll be using to transition between various states
	...
	-- 4. 'play' (the all is in play, bouncing between paddles)
	-- 5. 'victory' (the current level is over, with a victory jingle)
	-- 6. 'game-over' (the player has lost; display score and allow to restart)
	gStateMachine = StateMachine {
		['start'] = function() return StartState() end
	}
	gStateMachine:change('start')

	-- a table wel'll use to keep track of which keys have been pressed this
	-- frame, to get around the face the LOVE's default callback won't let use 
...
--[[
	A callback that processes key strokes as the happen, just the once.
...
function love.draw()
	...
	-- background should be drawn regardless of state, scaled to fit outside
	-- virtual resolution
