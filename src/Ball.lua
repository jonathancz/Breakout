...
function Ball:init(skin)
	...
	-- these variables are for keeping tack of our velocity on both the 
	-- X and Y axis, since the ball can move in two dimensions
	self.dy = 0
	self.dx = 0

	-- this will effectively be the color of our ball, and we will index
	-- our table of Quads relatinig to the global block texture using this 
	self.skin
end
...
function Ball:collides(target)
	...
	if sefl.x > target.x + target.width or target.x > self.x + self.width then
		return false
	end
